...
RT_FUNCTION void sample_light_area_sphere(float3& p, LightParameter &light, unsigned int &seed, AreaSample &sample)
{
...
RT_FUNCTION void sample_light_area(float3 &pos, LightParameter &light, unsigned int &seed, LightSample &sample)
{
...
RT_CALLABLE_PROGRAM void sample_light_point(float3 &pos, LightParameter &light, unsigned int &seed, LightSample &sample)
{
...
RT_CALLABLE_PROGRAM void sample_light_spot(float3 &pos, LightParameter &light, unsigned int &seed, LightSample &sample)
{
    float3 lightDir = light.position - pos;
    float lightDist = length(lightDir);
    float lightDistSq = lightDist * lightDist;
...
RT_CALLABLE_PROGRAM void sample_light_direction(float3 &pos, LightParameter &light, unsigned int &seed, LightSample &sample)
{
    sample.Li = light.emission;
    sample.wi = light.direction;
    sample.pdf = 1.0f;
    sample.lightDist = 9999999; // big number
}

RT_CALLABLE_PROGRAM void sample_light(float3 &pos, LightParameter &light, unsigned int &seed, LightSample &sample)
{
