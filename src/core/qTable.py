...
from core.quadtree import DTree
...
class QTable:
    def __init__(self, **kwargs):
        ...
        self.n_uv = kwargs.get("n_uv", 8)
        self.octree = kwargs.get("octree", None)
        self.max_quatree_count = kwargs.get("max_quatree_count", 1000)
    ...
    def update_quadtree(self, context):
        ...
        context['q_table_accumulated'] = Buffer.empty((self.n_s, self.n_a), dtype=np.float32, buffer_type='io', drop_last_dim=False)
        context['q_table_visit_counts'] = Buffer.empty((self.n_s, self.n_a), dtype=np.uint32, buffer_type='i', drop_last_dim=False)
