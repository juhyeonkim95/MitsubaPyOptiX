...
//-----------------------------------------------------------------------
{
    ...
    float F = DielectricFresnel(dot(V, H), state.eta > 1.0? 1 / state.eta : state.eta);
    float D = GTR2(dot(N, H), state.mat.roughness);
    float G = G1(dot(N, L), dot(H, L), state.mat.roughness) * G1(dot(N, V), dot(H, V), state.mat.roughness);
    ...
    weight *= pdf;
    return state.mat.albedo * weight;
}
...
//-----------------------------------------------------------------------
{
    float F = DielectricFresnel(abs(dot(V, H)), state.eta > 1.0? 1 / state.eta : state.eta);
    float D = GTR2(dot(N, H), state.mat.roughness);
    float G = G1(dot(N, L), dot(H, L), state.mat.roughness) * G1(dot(N, V), dot(H, V), state.mat.roughness);
    ...
    float pm = D * dot(N, H);
    pdf = pm * abs(dot(L, H)) / (denomSqrt * denomSqrt);
    pdf *= (1.0 - F);
    float weight = abs(dot(V, H)) * G * D / (abs(dot(V, N)) * pm);
    weight *= pdf;
    return state.mat.albedo * weight;
...
//-----------------------------------------------------------------------
{
    ...
    {
//        float cos_theta_i = abs(dot( V, N ));
//        float3 w_t;
//        const bool tir  = !optix::refract( w_t, -V, N, state.eta );
//        const float cos_theta_t = -optix::dot( N, w_t );
//        const float R  = tir  ? 1.0f : fresnel( cos_theta_i, cos_theta_t, state.eta );
